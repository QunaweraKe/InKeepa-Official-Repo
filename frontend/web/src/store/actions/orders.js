  ...
  axios
    ...
    .then((res) => {
      ...
      dispatch({
        ...
        payload: {
          text: "Wait for update on the cancellation request.",
          type: "success",
        },
      });
    })
...
// Order Create API
export const createOrder = (itemIds,table_id) => (dispatch, getState) => {
  // GET TOKEN FROM STATE
  const auth = getState().auth;
  const accessToken = auth.access;
  ...
  axios
    .post(
      ORDER_CREATE_API,
      JSON.stringify({ items: itemIds,table_id }),
      tokenConfig(accessToken)
    )
    .then((res) => {
      //Show the alert
      dispatch({
        type: SHOW_ALERT_MESSAGE,
        payload: {
          text: "Order Submitted,Check Status",
          type: "success",
        },
      });
      dispatch({
